...
class GrossProfitDetailReport(BaseCustomReport):
    report_name = "gross_profit_detail"
    ...
    def get_columns(self):
        return [
            ...
            {"fieldname": "qty", "label": "الكمية", "fieldtype": "Float", "width": 100},
            ...
            {"fieldname": "total_profit", "label": "إجمالي الربح", "fieldtype": "Currency", "width": 130},
        ]
    ...
    def get_data(self, filters):
        ...
        # Get buying rate from Stock Ledger Entry (average)
        query = (
            "SELECT si.posting_date, si.name, si.customer, "
            "sii.item_code, sii.item_name, sii.qty, sii.rate as selling_rate, "
            "COALESCE((SELECT AVG(valuation_rate) FROM `tabStock Ledger Entry` sle "
            "WHERE sle.item_code = sii.item_code AND sle.warehouse = sii.warehouse "
            "AND sle.posting_date <= si.posting_date ORDER BY sle.posting_date DESC LIMIT 1), "
            "sii.rate * 0.7) as buying_rate, "
            "sii.rate - COALESCE((SELECT AVG(valuation_rate) FROM `tabStock Ledger Entry` sle2 "
            "WHERE sle2.item_code = sii.item_code AND sle2.warehouse = sii.warehouse "
            "AND sle2.posting_date <= si.posting_date ORDER BY sle2.posting_date DESC LIMIT 1), "
            "sii.rate * 0.7) as profit_per_unit, "
            "(sii.rate - COALESCE((SELECT AVG(valuation_rate) FROM `tabStock Ledger Entry` sle3 "
            "WHERE sle3.item_code = sii.item_code AND sle3.warehouse = sii.warehouse "
            "AND sle3.posting_date <= si.posting_date ORDER BY sle3.posting_date DESC LIMIT 1), "
            "sii.rate * 0.7)) * sii.qty as total_profit "
