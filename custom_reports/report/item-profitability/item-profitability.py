...
class ItemProfitabilityReport(BaseCustomReport):
    report_name = "item-profitability"
    title = "Item Profitability"
    icon = "trending-up"

    def get_columns(self):
        return [{"fieldname":"item_code","label":"الصنف","fieldtype":"Link","options":"Item","width":150},{"fieldname":"item_name","label":"إسم الصنف","fieldtype":"Data","width":200},{"fieldname":"buying_rate","label":"سعر الشراء","fieldtype":"Currency","width":120},{"fieldname":"selling_rate","label":"سعر البيع","fieldtype":"Currency","width":120},{"fieldname":"profit_per_unit","label":"الربح للقطعة","fieldtype":"Currency","width":130},{"fieldname":"profit_margin_pct","label":"نسبة الربح","fieldtype":"Percent","width":120},{"fieldname":"total_qty","label":"عدد الوحدات","fieldtype":"Float","width":120},{"fieldname":"total_buying","label":"إجمالي سعر الشراء","fieldtype":"Currency","width":150},{"fieldname":"total_selling","label":"إجمالي سعر البيع","fieldtype":"Currency","width":150},{"fieldname":"total_profit","label":"الربحية","fieldtype":"Currency","width":130}]
    ...
    def get_data(self, filters):
        ...
        query = "SELECT sii.item_code, sii.item_name, AVG(sii.valuation_rate) as buying_rate, AVG(sii.rate) as selling_rate, AVG(sii.rate) - AVG(sii.valuation_rate) as profit_per_unit, ((AVG(sii.rate) - AVG(sii.valuation_rate)) / NULLIF(AVG(sii.rate), 0)) * 100 as profit_margin_pct, SUM(sii.qty) as total_qty, SUM(sii.valuation_rate * sii.qty) as total_buying, SUM(sii.amount) as total_selling, SUM(sii.amount) - SUM(sii.valuation_rate * sii.qty) as total_profit FROM `tabSales Invoice` si INNER JOIN `tabSales Invoice Item` sii ON sii.parent = si.name WHERE si.docstatus = 1 AND " + where_clause + " GROUP BY sii.item_code HAVING SUM(sii.qty) > 0 ORDER BY total_profit DESC LIMIT 1000"
