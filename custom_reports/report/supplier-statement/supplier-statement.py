...
class SupplierStatementReport(BaseCustomReport):
    report_name = "supplier-statement"
    ...
    def get_data(self, filters):
        conditions = []; values = []
        if filters.get("supplier"): conditions.append("party = %s"); values.append(filters["supplier"])
        ...
        query = "SELECT posting_date, voucher_type, voucher_no, debit, credit, 0 as balance, remarks FROM `tabGL Entry` WHERE party_type = 'Supplier' AND " + where_clause + " ORDER BY posting_date, creation LIMIT 1000"
